...
- ~packages~: External ocamlfind packages used at compile/link time
- ~ppx_packages~: PPX package names resolved via ~ocamlfind printppx~ (recommended for PPX workflows)
- ~compiler_flags~: Extra compiler flags (escape hatch for uncommon/custom invocations)
...
ocaml_package(
    ...
    ppx_packages=["js_of_ocaml-ppx"],  # Preferred PPX configuration
    # compiler_flags=[...]  # Escape hatch only
)
